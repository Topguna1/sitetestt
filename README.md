...
`data/sites.json`은 카테고리 키를 최상위 수준으로 갖는 객체입니다. 각 키는 해당 카테고리에 속한 사이트 배열을 나타내며, 항목은 아래 필드 규칙을 따릅니다.
...
데이터는 `data-loader.js`가 유효성 검사를 수행하며, 오류 발생 시 초기화가 중단되고 화면에 안내 메시지가 표시됩니다. 새 항목을 추가할 때는 해당 카테고리 배열에 객체를 추가하고, `category` 필드가 실제 위치한 배열과 일치하는지 확인하세요.
